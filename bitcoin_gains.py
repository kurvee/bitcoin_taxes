...
import urllib2
import urlparse
from pysqlite2._sqlite import Row
...
parser.add_argument('histories', metavar='FILE', nargs='+',
                   help='a csv, json file, or directory')
...
parser.add_argument('--assume_purchase_if_less_than', default=float('inf'))
...
class CsvParser(TransactionParser):
    ...
    def parse_file(self, filename):
        ...
        first = True
        self.rows = enumerate(csv.reader(open(filename)))
        for ix, row in self.rows:
            if not row or first:
                first = False
                continue
            else:
...
class BitstampParser2(CsvParser):
    ...
    def parse_row(self, row):
        type, timestamp, account, amount1, amount2, price, fee, subtype = row
        ...
        if type == 'Deposit' or type == 'Withdrawal':
            ...
            if amount1[1] == 'USD':
                ...
                raise ValueError, (amount1, amount2)
            btc = roundd(btc, 8)
            usd = roundd(usd, 2)
            if type == 'Deposit':
                return Transaction(timestamp, 'deposit', btc, usd, 0, fee)
            elif type == 'Withdrawal':
                return Transaction(timestamp, 'withdraw', -btc, -usd, 0, fee)
        elif subtype == 'Buy' or subtype == 'Sell':
            if subtype == 'Buy':
                btc = roundd(amount1[0], 8)
                usd = -roundd(amount2[0], 2)
            else:
                btc = -roundd(amount1[0], 8)
                usd = roundd(amount2[0], 2)
            return Transaction(timestamp, 'trade', btc, usd, price, fee)
...
class BitfinexParser(TransactionParser):
    expected_files = ['ledger-usd.csv', 'ledger-btc.csv']
    
    class LedgerParser(CsvParser):
        expected_headers = 'Currency,Description,Amount,Balance,Date'
        
        def parse_row(self, row):
            # If currency is BTC, ignore all but deposits and withdrawals.
            # If currency is USD, add fee on line after (above) Exchange to transaction.
            currency, description, amount, balance, timestamp = row
            timestamp = time.strptime(timestamp, '%Y-%m-%d %H:%M:%S')
            desc = re.split(' ', description)
            # Deposit / Withdrawals
            if desc[0] == 'Deposit' and currency == 'USD':
                return Transaction(timestamp, 'deposit', 0.0, roundd(amount, 2), 0)
            elif desc[0] == 'Deposit' and currency == 'BTC':
                return Transaction(timestamp, 'deposit', roundd(amount, 8), 0.0, 0)
            elif desc[0] == 'Withdrawal' and currency == 'USD':
                return Transaction(timestamp, 'withdraw', 0.0, -roundd(amount, 2), 0)
            elif desc[0] == 'Withdrawal' and currency == 'BTC':
                return Transaction(timestamp, 'withdraw', -roundd(amount, 8), 0.0, 0)
            # Ignore intra-account wallet transfers
            elif desc[0] == 'Transfer':
                pass
            # Trades
            elif desc[0] == 'Exchange':
                assert(currency == 'BTC')
            elif desc[0] == 'Trading':
                assert(currency == 'USD')
                fee = abs(float(amount))
                currency2, description2, amount2, balance2, timestamp2 = self.rows.next()[1]
                desc2 = re.split(' ', description2)
                price = decimal.Decimal(desc2[6])
                if desc2[2] == 'BTC':
                    btc = -roundd(float(desc2[1]), 8)
                    usd = roundd(float(amount2), 2)
                elif desc2[2] == 'USD':
                    btc = roundd(float(desc2[1]), 8)
                    usd = -roundd(float(amount2), 2)
                return Transaction(timestamp, 'trade', btc, usd, price, fee)
    ...
    def can_parse(self, filemname):
        if not os.path.isdir(filename):
            return False
        else: 
            return BitfinexParser.LedgerParser().can_parse(os.pathsep.join((filename, BitfinexParser.expected_files[0]))) \
                    and LedgerParser().can_parse(os.pathsep.join((filename, BitfinexParser.expected_files[1])))
                    
    def parse_file(self, filename):
        ledgers = [ t for t in BitfinexParser.LedgerParser().parse_file(os.sep.join((filename, BitfinexParser.expected_files[0]))) ]
        ledgers.extend([ t for t in BitfinexParser.LedgerParser().parse_file(os.sep.join((filename, BitfinexParser.expected_files[1]))) ])
        ledgers.sort()
...
def main(args):
    ...
    parsers = [BitstampParser(), BitstampParser2(), BitfinexParser(), MtGoxParser(), BitcoindParser(), CoinbaseParser(), ElectrumParser(), DbDumpParser(), BitcoinInfoParser(), TransactionParser()]
    all = []
    for file in args.histories:
        for parser in parsers:
            if (os.path.isdir(file) and parser.can_parse_dirs()) or (os.path.isfile(file) and parser.can_parse(file)):
                print file, parser
            ...
            if t.id in external:
                ...
                if btc == 0:
                    ...
                    if t.type == 'fee':
                        ...
                        if -btc >= float(parsed_args.assume_purchase_if_less_than):
